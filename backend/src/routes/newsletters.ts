...
router.get('/admin/all', authenticateToken, isAdmin, async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/:id/send', authenticateToken, isAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const newsletter = await Newsletter.findById(req.params.id);
