   ...
   const handleSearch = async () => {
      if (!searchData.level && !searchData.destination && !searchData.field) {
         toast.error("Please fill at least one field", {
            description:
               "Select a study level, destination, or field of study to search.",
         });
         return;
      }
      ...
      try {
         const query =
            `${searchData.level} ${searchData.destination} ${searchData.field}`.trim();
         await searchesAPI.create(query, searchData);

         toast.success("Search Saved!", {
            description:
               "Opening consultation form for personalized recommendations.",
         });
      ...
      <Card className="w-full max-w-5xl mx-auto -mt-8 relative z-20 p-6 md:p-8 shadow-2xl bg-card border-none">
         ...
         <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Select
               ...
               onValueChange={(value) =>
                  setSearchData({ ...searchData, level: value })
               }
            >
